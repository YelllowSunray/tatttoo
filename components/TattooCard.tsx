...
export function TattooCard({ tattoo, artistName, artistLocation }: TattooCardProps) {
  ...
  return (
    <div className="group relative aspect-square overflow-hidden bg-black transition-all duration-500 hover:opacity-95">
      <Image
        src={tattoo.imageUrl}
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/0 to-black/0 opacity-0 transition-opacity duration-500 group-hover:opacity-100">
        <div className="absolute bottom-0 left-0 right-0 p-6">
          ...
          {/* Artist name */}
          {artistName && (
            <p className="mb-2 text-xs font-medium text-white/90 uppercase tracking-wider">
              {artistName}
            </p>
          )}
          
          {/* Price and Location */}
          <div className="mb-3 flex flex-wrap gap-3 text-xs text-white/80">
            {tattoo.price && (
          ...
          <button
            onClick={handleLike}
            disabled={isToggling}
            className={`flex items-center gap-2 rounded-full px-5 py-2.5 text-xs font-medium transition-all duration-300 uppercase tracking-wider ${
              isLiked
      ...
      {/* Like indicator when liked but not hovering */}
      {isLiked && (
        <div className="absolute top-4 right-4 rounded-full bg-white p-2 shadow-lg">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            fill="currentColor"
